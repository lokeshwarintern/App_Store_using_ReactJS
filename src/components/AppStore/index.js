...
class AppStore extends Component {
  ...
  render() {
    const {activeTabId, searchInputVal} = this.state
    const fiteredAppList = appsList.filter(
      eachItem => eachItem.category === activeTabId,
    )
    const searchResults = fiteredAppList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInputVal.toLowerCase()),
    )

    return (
      <div className="bg-container">
        <div className="app-store-card">
          <h1 className="heading-ele">App Store</h1>
          ...
          <ul className="ul-tablist-container">
            {tabsList.map(eachItem => (
              <TabItem
                key={eachItem.tabId}
                tabBtnEle={eachItem}
                btnClick={this.onChangeTabItem}
                isActive={eachItem.tabId === activeTabId}
              />
            ))}
          </ul>
          <ul className="ul-app-item-card">
